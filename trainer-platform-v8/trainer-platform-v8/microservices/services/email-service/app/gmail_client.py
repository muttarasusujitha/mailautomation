...
def _html_template(body: str, from_name: str, from_email: str, tracking_url: str = "") -> str:
    ...
    if _is_trainer_reply(body):
        from_email = _resolve_sender_email(settings.FROM_EMAIL or settings.GMAIL_USER or from_email)
...
def send_gmail_oauth(
    ...
    try:
        ...
        if _is_trainer_reply(body):
            sender_email = _resolve_sender_email(settings.FROM_EMAIL or settings.GMAIL_USER or sender_email)
    ...
    for idx, candidate in enumerate(candidates):
        ...
        try:
            ...
            if "gmail" in host.lower() and idx < len(candidates) - 1:
                continue
            if "gmail" in host.lower() and not pwd:
