...
PO_DATE_PATTERN = r"\b(?:date|po\s*date)\s*[:# \-]?\s*(\d{1,2}[-/]\d{1,2}[-/]\d{2,4}|\d{1,2}\w*\s+[A-Za-z]+\s+\d{4})"  # nosonar
PO_TERMS_PATTERN = r"\b(?:terms|payment\s*terms)\s*[:# \-]?\s*([A-Za-z0-9 ,./-]{20,80})(?=\s+(?:ref|project|vendor|$))"  # nosonar
DATE_RANGE_PATTERN = r"\b(?:start\s*date)\s*[-: ]?\s*(\d{1,2}[-/]\d{1,2}(?:[-/]\d{2,4})?)\s+(?:to|and|-)\s+(\d{1,2}[-/]\d{1,2}(?:[-/]\d{2,4})?)"  # nosonar
TIME_DAY_PATTERN = r"\b(?:\d{1,2}(?::\d{2})?\s*(?:am|pm)?|today|tomorrow|mon|tue|wed|thu|fri|sat|sun)\b"  # nosonar
...
def _parse_toc_knowledge_blocks(text: str) -> list:  # nosonar
...
async def _admin_toc_domain_for(db, name: str) -> Optional[dict]:  # nosonar
    key = _toc_key(name)
    if not key:
        return None
...
async def generate_training_toc(payload: dict, request: Request):  # nosonar
...
async def send_toc_email(payload: dict):  # nosonar
    toc_id = payload.get("toc_id")
    if not toc_id:
        raise HTTPException(400, "toc_id is required")
    db = get_db()
...
async def auto_generate_toc(payload: dict, request: Request):  # nosonar
    db = get_db()
...
def _render_invoice_html(invoice_doc: dict) -> str:  # nosonar
    esc = _html.escape
...
def _simple_invoice_pdf_bytes(invoice_doc: dict) -> bytes:  # nosonar
    company = _invoice_display_company(invoice_doc)
    client = invoice_doc.get("client") or {}
    requirement = invoice_doc.get("requirement") or {}
    commercials = invoice_doc.get("commercials") or {}
