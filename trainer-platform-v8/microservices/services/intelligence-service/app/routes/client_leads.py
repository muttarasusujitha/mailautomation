...
def _recency_start(recency: Optional[str]) -> Optional[datetime]:
    now = datetime.utcnow()
    key = (recency or "").strip().lower()
    if key in {"today", "day", "24h"}:
        return now - timedelta(days=1)
    if key in {"week", "7d", "this_week"}:
        return now - timedelta(days=7)
    if key in {"month", "30d", "this_month"}:
        return now - timedelta(days=30)
    if key in {"year", "current_year", "this_year"}:
...
async def list_client_leads(
    ...
    domain: Optional[str] = None,
    q: Optional[str] = None,
    recency: Optional[str] = Query("week"),
    ...
    start = _recency_start(recency)
    if start:
        query["$and"] = [
            {"$or": [
                {"created_at": {"$gte": start}},
                {"last_seen_at": {"$gte": start}},
    ...
    cursor = db["client_leads"].find(query, {"_id": 0}).sort([("published_at", -1), ("post_date", -1), ("created_at", -1)]).skip(skip).limit(page_size)
