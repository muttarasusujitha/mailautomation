...
def poll_inbox(self):
    ...
    try:
        # Raw Gmail message reads consume the per-user API quota.  Keep each
        # automatic run bounded; the next poll picks up remaining messages.
        resp = httpx.post(url, json={"since_days": 3, "max_messages": 25}, timeout=300)
