...
export default defineConfig(({ mode }) => {
  ...
  const coreApiTarget =
    ...
    'http://ts-gateway:8000'
  const intelligenceServiceTarget = env.VITE_INTELLIGENCE_SERVICE_PROXY_TARGET || 'http://ts-gateway:8000'
  const emailServiceTarget = env.VITE_EMAIL_SERVICE_PROXY_TARGET || 'http://ts-gateway:8000'
  const trainerServiceTarget = env.VITE_TRAINER_SERVICE_PROXY_TARGET || 'http://ts-gateway:8000'
